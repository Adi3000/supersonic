...
public class Schema37 extends Schema {
    ...
    public void execute(JdbcTemplate template) {
        ...
        if (template.queryForInt("select count(*) from role where id = 9") == 0) {
            ...
            template.execute("insert into users_role select distinct u.username, 9 from users u");
        ...
        if (template.queryForInt("select count(*) from role where id = 10") == 0) {
            ...
            template.execute("insert into users_role " +
                             "select distinct u.username, 10 from users u, users_role ur " +
        ...
        if (!columnExists(template, "changed", "music_folder")) {
            ...
            template.execute("alter table music_folder add changed timestamp default CURRENT_DATE not null");
        ...
        if (!columnExists(template, "changed", "internet_radio")) {
            ...
            template.execute("alter table internet_radio add changed timestamp default CURRENT_DATE not null");
            LOG.info("Database column 'internet_radio.changed' was added successfully.");
        }

        if (!columnExists(template, "changed", "users_settings")) {
            LOG.info("Database column 'users_settings.changed' not found.  Creating it.");
            template.execute("alter table users_settings add changed timestamp default CURRENT_DATE not null");
            LOG.info("Database column 'users_settings.changed' was added successfully.");
        }
