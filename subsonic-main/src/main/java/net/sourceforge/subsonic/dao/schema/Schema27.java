...
public class Schema27 extends Schema{
    ...
    public void execute(JdbcTemplate template) {
        ...
        if (template.queryForInt("select count(*) from version where version = 3") == 0) {
            ...
            //TODO Implement a solution for this other databases
            if(HSQL_DB == this.getDbType() ){
        ...
        if (!columnExists(template, "bytes_streamed", "users")) {
            LOG.info("Database columns 'users.bytes_streamed/downloaded/uploaded' not found.  Creating them.");
            template.execute("alter table users add bytes_streamed bigint default 0 not null");
            template.execute("alter table users add bytes_downloaded bigint default 0 not null");
            template.execute("alter table users add bytes_uploaded bigint default 0 not null");
            LOG.info("Database columns 'users.bytes_streamed/downloaded/uploaded' were added successfully.");
        }
    }
}
